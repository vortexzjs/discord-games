...
const { disableButtons, shuffleArray, formatMessage, buttonStyle, ButtonBuilder } = require('../../utils/utils.js');
...
module.exports = class MatchPairs extends events {
  constructor(options = {}) {
    ...
    if (!options.message) throw new TypeError('NO_MESSAGE: Nenhuma opção de mensagem foi fornecida.');
    if (typeof options.message !== 'object') throw new TypeError('INVALID_MESSAGE: message option must be an object.');
    if (typeof options.isSlashGame !== 'boolean') throw new TypeError('INVALID_COMMAND_TYPE: isSlashGame option must be a boolean.');


    if (!options.embed) options.embed = {};
    if (!options.embed.title) options.embed.title = 'Jogo da memoria';
    if (!options.embed.color) options.embed.color = '#5865F2';
    if (!options.embed.description) options.embed.description = '**Clique nos botões para combinar emojis com seus pares.**';

    if (!options.timeoutTime) options.timeoutTime = 60000;
    if (!options.emojis) options.emojis = ['🍉', '🍇', '🍊', '🍋', '🥭', '🍎', '🍏', '🥝', '🥥', '🍓', '🍒', '🫐', '🍍', '🍅', '🍐', '🥔', '🌽', '🥕', '🥬', '🥦'];
    if (!options.winMessage) options.winMessage = '**Você ganhou! Você virou um total de: `{tilesTurned}` peças.**';
    if (!options.loseMessage) options.loseMessage = '**Você perdeu! Você virou um total de: `{tilesTurned}` peças.**'; 
